...
public class DownloadScheduler extends Thread {
            ...
            if (downloadVideo.videoList.size() < 3&&downloadVideo.startPlayingTime==0)
            {
                if (downloadStarted == false)
                {
                   ...
                   // Log.i("scheduler running","scheduler running");
                    ...
                    if(parseMPD.DownloadMPD())
                    {
                        int initialsegment = parseMPD.GetInitialSegment();
                        if(initialsegment!=-1)
                        {
                          //  Log.i("foundSegScheduler","foundSegScheduler");
                            String firstSegmentPath = parseMPD.basePath + "/" + parseMPD.highFolder + "/" + "Seg" + String.valueOf(initialsegment) + ".mp4";
                            //Log.i("basepath",firstSegmentPath);
                            //String firstSegmentPath = parseMPD.basePath +"/Seg"+String.valueOf(initialsegment)+"/" + "high" + "/" + "Seg" + String.valueOf(initialsegment) + ".mp4";
                            ...
                            for (int i = 0; i < downloadVideo.synchListrequestFail.size(); i++)
                            {
                                ...
                                if (failType == 0)
                                {
                                    ...
                                    if (parseMPD.ParseMPD(requestNo)) {
                                        ...
                                        i--;
                                        String path = parseMPD.basePath + "/"+"Seg" + String.valueOf(r.requestNo)+ "/" + parseMPD.highFolder + "/" + "Seg" + String.valueOf(r.requestNo) + ".mp4";
                ...
                if (downloadVideo.videoStarted==false&&downloadVideo.segmentbeforestart ==-1)
                {
                 ...
                 //   Log.i("request seg b4 start",String.valueOf(requestNo));
                    if (parseMPD.ParseMPD(requestNo))
                    {
                        ...
                        String path=parseMPD.basePath + "/"+"Seg" + String.valueOf(req.requestNo)+ "/" + parseMPD.highFolder + "/" + "Seg" + req.requestNo + ".mp4";
                    ...
                    else
                    {
                        ...
                        HttpResponse resp = httpClient.execute(new HttpGet(parseMPD.basePath+"/"+"MPD1.xml"));
                                    ...
                                    if (System.currentTimeMillis() - re.requestSentTime >= 200) {
                                        ...
                                        else if(req.requestFailType==1)
                                        {
                                            ...
                                            if(re.quality=="High")
                                                ...
                                                folder=parseMPD.lowFolder;
                                                //if(!folder.equals(""))
                                                path = parseMPD.basefilePath + "/"+"Seg" + String.valueOf(req.requestNo)+ "/" + folder + "/" + "Seg" + req.requestNo + ".mp4";
                                            }
                                            {
                                                downloadVideo.synchListrequestFail.remove(i);
                                                //String path = parseMPD.basefilePath + "/" + folder + "/" + "Seg" + req.requestNo + ".mp4";
    ...
    boolean DownloadParseSegmentsBeforePlay(int segmentNo)
    {
        ...
        if(parseMPD.ParseMPD(segmentNo))
        {
            ...
            String path=parseMPD.basePath+ "/"+"Seg" + String.valueOf(segmentNo)+"/"+parseMPD.highFolder+"/"+"Seg"+String.valueOf(segmentNo)+".mp4";
            //download high quality
            HTTPRequests req=new HTTPRequests();
                ...
                {*/
                    if(parseMPD.DownloadMPD())
                    {
                        if(parseMPD.ParseMPD(segmentNo))
                        {
                            ...
                            String path = parseMPD.basePath +"/"+"Seg" + String.valueOf(segmentNo)+ "/" + parseMPD.highFolder + "/" + "Seg" + segmentNo + ".mp4";
                            //download high quality
                            HTTPRequests req = new HTTPRequests();
                            req.requestNo = segmentNo;
                            req.requestSentTime = reqsentTime;
    ...
    String AdaptationControl(HTTPRequests request,int high,int mid,int low)
    {
        long expectedTimeTodownloadInitially = System.currentTimeMillis() + 3 * downloadVideo.segmentDuration;
            ...
            if (newDownloadTime < p.playWindowStart + (3 * downloadVideo.segmentDuration)) {
                ...
                String url=parseMPD.basefilePath+"/"+"Seg" + String.valueOf(request.requestNo)+"/"+parseMPD.highFolder+"/"+String.valueOf(request.requestNo)+".mp4";
                return url;
            } else {
                ...
                if (newDownloadTimemed < p.playWindowStart + (3 * downloadVideo.segmentDuration)) {
                    ...
                    String url=parseMPD.basefilePath+"/"+"Seg"+String.valueOf(request.requestNo)+"/"+parseMPD.midFolder+"/"+String.valueOf(request.requestNo)+".mp4";
                    //downloadVideo.synchListrequestPending.add(request);
                    return url;
                } else
                {
                    ...
                    request.quality = "Low";
                    String url=parseMPD.basefilePath+"/"+"Seg"+String.valueOf(request.requestNo)+"/"+parseMPD.lowFolder+"/"+String.valueOf(request.requestNo)+".mp4";
                    //downloadVideo.synchListrequestPending.add(request);
                    return url;
                }
                ...
                if(downloadVideo.startplayingTimeSet==true)
                {
                    if(expectedTimeTodownloadInitially<=p.playWindowStart+(3*downloadVideo.segmentDuration)&&expectedTimeTodownloadInitially>System.currentTimeMillis())
                    {
                        ...
                        if(newDownloadTime<p.playWindowStart+(3*downloadVideo.segmentDuration))
                        {
                            ...
                            //downloadVideo.synchListrequestFail.add(req);
                            return parseMPD.basePath+"/"+"Seg"+String.valueOf(reqNo)+"/"+parseMPD.highFolder+"/"+"Seg"+String.valueOf(reqNo)+".mp4";
                        }
                        else
                        {
                            ...
                            if(newDownloadTimemed<p.playWindowStart+(3*downloadVideo.segmentDuration))
                            {
                                ...
                                //downloadVideo.synchListrequestFail.remove(0);
                                //downloadVideo.synchListrequestFail.add(req);
                                return parseMPD.basePath+"/"+"Seg"+String.valueOf(reqNo)+"/"+parseMPD.midFolder+"/"+"Seg"+String.valueOf(reqNo)+".mp4";
                            }
                            else
                            {
                               ...
                               // if(newDownloadTimemed<p.playWindowStart+(3*downloadVideo.segmentDuration))
                                {
                                    ...
                                    //downloadVideo.synchListrequestFail.remove(0);
                                    //downloadVideo.synchListrequestFail.add(req);
                                    return parseMPD.basePath+"/"+"Seg"+String.valueOf(reqNo)+"/"+parseMPD.lowFolder+"/"+"Seg"+String.valueOf(reqNo)+".mp4";
                                }
                            }
